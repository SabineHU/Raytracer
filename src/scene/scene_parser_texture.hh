...
shared_texture parse_lambertian_texture(const nlohmann::json& json)
{
...
shared_texture parse_metal_texture(const nlohmann::json& json)
{
    check_missing_field(json, "color");
...
shared_texture parse_image_texture(const nlohmann::json& json)
{
...
PerlinNoiseType parse_perlin_noise_type(const nlohmann::json& json)
{
    ...
    std::cerr << "Unsupported noise type \"" << json << "\"!\n";
    exit(1);
}

shared_texture parse_perlin_noise_texture(const nlohmann::json& json)
{
...
void parse_texture_coeffs(const nlohmann::json& json, shared_texture texture)
{
...
void parse_texture_type(const nlohmann::json& json, shared_texture texture)
{
...
// TODO: @sebmenozzi avoid undeclared function
shared_texture parse_texture(const nlohmann::json& json);

shared_texture parse_checkerboard_texture(const nlohmann::json& json)
{
...
shared_texture parse_strip_texture(const nlohmann::json& json)
{
    check_missing_field(json, "color1");
    check_missing_field(json, "color2");
...
shared_texture parse_texture(const nlohmann::json& json)
{
    ...
    {
        std::cerr << "Unsupported texture type \"" << texture_type << "\"!\n";
        exit(1);
    }
...
Noise parse_noise(const nlohmann::json& json)
{
    check_missing_field(json, "type");
    check_missing_field(json, "scale");
...
void parse_bump_mapping(const nlohmann::json& json, shared_object object)
{
...
void parse_bump_mappings(const nlohmann::json& json, shared_object object)
{
    for (auto bump_mapping : json)
